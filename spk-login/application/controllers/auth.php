...
class auth extends CI_Controller
{
    ...
    private function _login()
    {
        ...
        if ($user) {
            ...
            if ($user['is_active'] == 1) {
                ...
                $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Email ini belum belum diaktivasi!</div>');
                redirect('auth');
            }
        } else {
    ...
    private function _sendEmail($token, $type)
    {
        ...
        if ($type == 'verify') {
            ...
            $this->email->message('Klik link berikut ini untuk memverifikasi akun Serikat Pekerja Kampus anda: <a href="' . base_url() . 'auth/verify?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '">Aktivasi Akun</a>');
        } else if ($type == 'forgot') {
